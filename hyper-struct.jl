...
const TERM_PRECISION = 80 # MAX_ORDER for calculations
...
function isless(x::Hyper, y::Hyper)
    ...
    for i in 1:max(nx, ny)
        if i > nx return true end
        if i > ny return false end
        rx, ex = tx[i]
        ry, ey = ty[i]
        if ex > ey return true end
        if ex < ey return false end
        if rx < ry return true end
        if rx > ry return false end
    end
    return false
end
...
∂(f::Function) = Closure(x -> simplify((f(x+ε) - f(x-ε)) / (2ε)))
∂(c::Closure) = Closure(x -> simplify((c.f(x+ε) - c.f(x-ε)) / (2ε)))
# ∂(f::Function) = x -> denoise(simplify((f(x+ε) - f(x-ε)) / (2ε)))

# (w::∂Wrapper)(x) = simplify((w.f(x + ε) - w.f(x - ε)) / (2ε))
# Base.show(io::IO, w::∂Wrapper) = print(io, "∂(", w.f, ")")
(w::Closure)(x) = w.f(x)
Base.show(io::IO, w::Closure) = print(io, "ƒ(", w.f, ")")
...
≈(f::Closure, y::Number) = all(f.f(x) ≈ y for x in (-1.0, 0.0, 1.0)) # lol
≈(f::Closure, y::Hyper) = all(f.f(x) ~ y for x in (-1.0, 0.0, 1.0)) # lol
≈(h::Hyper, f::Closure) = all(h ~ f.f(x) for x in (-1.0, 0.0, 1.0)) # lol
≈(f::Closure, g::Function) = f.f==g || all(f.f(x) ~ g(x) for x in (-1.0, 0.0, 1.0)) # lol
≈(f::Closure, g::Closure) = f.f≈g.f
# ≈(f::Function, g::Function) = f==g || all(f(x) ≈ g(x) for x in (-1.0, 0.0, 1.0)) # lol
...
Base.:/(c::Closure, y::Hyper) = Closure(x -> c.f(x) / y)
...
function plot_ascii(f::Closure; xlim=(-5,5), width=60, height=20)
    plot_ascii(f.f; xlim=xlim, width=width, height=height)
end

# plot_ascii(sin; xlim=(-π, π))
# plot_ascii(st(∂(sin)); xlim=(-π, π))
# plot_ascii(st(∂(square)); xlim=(-π, π))
# plot_ascii(∂(square); xlim=(-π, π))
# @assert sin(x::Hyper) ≈ sin(x)
...
∫(c::Closure) = Closure(x -> c.f(x+ε) * ε - c.f(x))

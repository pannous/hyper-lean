...
\begin{center}\small\url{https://files.pannous.com/hyperreal-exercises-dark.pdf}\end{center}
...
booklet \href{https://files.pannous.com/hyperreal-integral-exercises-dark.pdf}{\emph{Twenty Exercises in Probability on Ordinary
Intervals}}:
\begin{center}\small\url{https://files.pannous.com/hyperreal-integral-exercises-dark.pdf}\end{center}
...
of an interval, and $\eps\cdot \omega = 1$. \textbf{Partial.}
...
\textbf{Solution.} Normalization is Exercise 13's sum with a factor $2$, giving
$1 - \eps$; on the half-open convention the exactly normalized triangular
density is $p(x) = 2x/(1-\eps)$. In either case
...
infinitesimals of equal order. \textbf{Now} for the ratio; \textbf{Partial} for the exact
...
integral over disjoint regions, and order comparison. \textbf{Now} for the two point
values; \textbf{Partial} for the normalization integral.
...
\textbf{Solution.} With $x = k\cdot \eps$ and the finite power-sum identities,
...
$\omega = 1/\eps$. \textbf{Research:} an index genuinely ranging over $\omega$
dots does not exist in the current Lean model, even though the closed forms
are ordinary $\Rstar$ values.
...
$st$-invariance. \textbf{Research} for the general statement; \textbf{Now} for the
$f(x) = x$ instance, whose three values are explicit $\Rstar$ numbers.

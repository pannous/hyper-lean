...
Produces two standalone LaTeX documents (and PDFs, if pdflatex is available):
  paper/hyperreal-exercises.tex           problems only, no solutions
  paper/hyperreal-exercise-solutions.tex  worked solutions
Both omit the theory, which is linked as a separate paper.
"""
...
THEORY_URL = "https://files.pannous.com/hyperreals.pdf"
EXERCISES_URL = "https://files.pannous.com/hyperreal-exercises.pdf"
SOLUTIONS_URL = "https://files.pannous.com/hyperreal-exercise-solutions.pdf"
...
def document(title, intro, exercises, body_sections, closing):
    parts = [PREAMBLE % {"title": title}, intro]
...
def main():
    ...
    links = {"theory": THEORY_URL, "repo": REPO_URL}
...
companion booklet \href{%s}{\emph{Twenty Exercises in Algebraic Probability ---
Solutions}}:
\begin{center}\small\url{%s}\end{center}
...
\end{itemize}
The modules use
coefficient-wise algebraic equality and explicitly proved monomial quotients;
their headline theorems have been audited with \texttt{\#print axioms} and use
neither \texttt{sorryAx}, nor the unsafe raw-list equality axiom, nor
unrestricted mixed-order inversion. For a \textbf{Partial} exercise, a checked
kernel means the displayed closed-form algebra is proved while the reusable
event or random-variable abstraction remains to be built. For a
\textbf{Research} exercise, the exact finite algebra and a proof-carrying
interface for the missing analytic or hyperfinite fact are checked;
constructing an instance of that interface is deliberately not passed off as
solved.
"""
            % {"ex": EXERCISES_URL},
            exercises,

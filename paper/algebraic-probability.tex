...
\subsection{Finite HyperLists as the foundation}
A HyperList is a finite list of coefficient--exponent pairs. For integer
exponents, its value is
\[
 [(a_1,n_1),\ldots,(a_m,n_m)]\longmapsto
 \sum_{i=1}^m a_i\omega^{n_i}.
\]
Appending lists adds values; convolution multiplies coefficients and adds
exponents. Sorting, collecting equal exponents, and deleting zero terms
change the presentation, not its value. Literal list equality is therefore
not the equality of numbers.

\begin{theorem}[Finite-list fraction presentation]
Every element of $K(\omega)$, for a field $K$, is a quotient of two finite
integer-exponent HyperLists with nonzero denominator. Identifying such
quotients by equality after cross multiplication gives a field isomorphic
to $K(\omega)$. Addition and multiplication agree with finite-list
arithmetic; a nonzero reciprocal swaps the whole numerator and denominator.
\end{theorem}
\begin{proof}
The numerator and denominator polynomials of a rational function are finite
lists of their coefficients and nonnegative exponents. Conversely, each
finite Laurent expression belongs to $K(\omega)$. Evaluation commutes with
addition and convolution; the field laws and the quotient identification
follow from the corresponding rational-function identities.
\end{proof}

For example, the closed unit interval is $[(1,0),(1,-1)]$.
The calibrated point probability has the two-list presentation
\[
 \eta=\frac{[(1,-1)]}{[(1,0),(1,-1)]}.
\]
Convolution gives the square and cube contents. The unit disk is
$[(\pi,0),(\pi,-1),(1,-2)]$, while the solid ball is
$[(4\pi/3,0),(2\pi,-1),(4,-2),(1,-3)]$.
No infinite expansion of a reciprocal is required.

The Lean development checks this presentation and implements the normalized
integral below by finite-list fraction additions, products, and reciprocal
of the total. Its interpreted value equals the existing context integral
for every finite context and observable over $K(\omega)$.
Real geometric coefficients use $K=\R$; the original rational-coefficient
backend alone cannot express $\pi$ or $\sqrt2$.

A separate audited adapter interprets the original rational-exponent
HyperLists in the finite group algebra $\mathbb Q[\mathbb Q]$ and its fraction
field. It proves preservation of normalization, addition, negation, and
multiplication without using the legacy raw-list field axioms. This
rational-exponent fraction field is not identified with $\R(\omega)$; its ordered
probability interface is not yet formalized. The integer-exponent
presentation already covers all probability formulas in this manuscript.
...
The companion code uses exact finite-list fractions and a separately audited
adapter for the historical list backend. The current audited path uses the following modules:
\begin{itemize}
\item \path{Hyper/HyperListFoundation.lean}: finite-list fractions, field
quotient, and equivalence with rational functions.
\item \path{Hyper/HyperListProbability.lean}: list arithmetic for the
normalized integral and explicit geometric presentations.
\item \path{Hyper/HyperListSemantics.lean}: audited interpretation of
original rational-exponent list arithmetic.
...
Finite-list fractions and integral interpretation & Lean-checked \\
...
\url{https://github.com/pannous/hyper-lean}; the accompanying checkout contains the finite-list foundation and its
separate legacy-adapter audit. Run \texttt{bash test.sh} with the pinned
